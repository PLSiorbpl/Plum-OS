...
{
    return (static_cast<uint64_t>(limit & 0x0000FFFF) << 0) |
...
void init_tss() {
    ...
    gdt[7] = static_cast<uint64_t>(limit & 0xFFFF) | ((base & 0xFFFFFF) << 16) |
        (static_cast<uint64_t>(ACCESS_TSS) << 40) | ((base >> 24 & 0xFF) << 56);
    gdt[8] = base >> 32;
